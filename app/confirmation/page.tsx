...
export default function ConfirmationPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-20 text-center">
      ...
      {/* Success icon */}
      <div
        className="w-20 h-20 bg-[#16A34A] rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg"
        role="img"
      ...
      <h1 className="font-[var(--font-dm-sans)] text-4xl font-bold text-[#111827] mb-2">
        Order Confirmed!
      </h1>
      {name && (
        <p className="text-lg text-gray-500 mb-1">
          Thank you, <span className="font-semibold text-[#111827]">{name}</span>!
        </p>
      )}
      <p className="text-gray-400 text-sm mb-8">
        Order <span className="font-mono font-semibold text-[#111827]">{orderNumber}</span> · {planData?.name} Plan · ₱{planData?.price}/month
      </p>

      {/* Order details */}
      <div className="bg-white rounded-2xl border border-gray-100 p-6 text-left shadow-sm mb-6">
        <h2 className="font-[var(--font-dm-sans)] font-semibold text-[#111827] mb-4">Your Box</h2>
        {items.length > 0 ? (
          <ul className="space-y-3" aria-label="Items in your order">
            {items.map((item) => (
              <li key={item.id} className="flex items-center gap-3 text-sm">
                ...
                <span className="text-[#111827] flex-1">{item.name}</span>
      ...
      {/* CSR thank-you */}
      <div className="bg-[#F0FDF4] rounded-2xl p-6 mb-8 text-left">
        <h3 className="font-[var(--font-dm-sans)] font-semibold text-[#111827] mb-2">🌱 Thank You for Supporting Wellness &amp; CSR</h3>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-8 text-left text-sm">
        ...
        <ol className="space-y-2 text-gray-500">
          <li className="flex gap-2"><span className="text-[#16A34A] font-bold">1.</span> You'll receive a confirmation email within 24 hours.</li>
          <li className="flex gap-2"><span className="text-[#16A34A] font-bold">2.</span> Your box is assembled and packed by local partners.</li>
          <li className="flex gap-2"><span className="text-[#16A34A] font-bold">3.</span> Delivery within Metro Manila in 3–5 business days.</li>
        </ol>
      </div>

      <div className="flex flex-col sm:flex-row gap-4 justify-center">
        <Link href="/">
          <button className="min-h-[48px] border-2 border-[#16A34A] text-[#16A34A] px-6 py-3 rounded-full font-semibold hover:bg-[#16A34A] hover:text-white transition-colors">
            Back to Home
          </button>
        </Link>
        <Link href="/plans">
          <button className="min-h-[48px] bg-[#16A34A] text-white px-6 py-3 rounded-full font-semibold hover:bg-[#15803D] transition-colors">
            Build Another Box →
          </button>
        </Link>
      </div>
